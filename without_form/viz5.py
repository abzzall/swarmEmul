...
def episode_gui(env: Env, w1, w2,  window_width=WINDOW_SIZE, window_height=WINDOW_SIZE, min_agent_size=0, fps=FPS,
                draw_way=False, stop_on_finish=False):
    pygame.init()
    screen = pygame.display.set_mode((window_width, window_height), HWSURFACE | DOUBLEBUF | RESIZABLE)
    pygame.display.set_caption('Simulator of the swarm motion')
...
if __name__ == '__main__':
    ...
    file_to_check=open('test5.txt', 'w')
    run = 0
    dir_name = os.path.join('../..', 'run5')
    # for w2 in W:
    #     # print('w1=' + str(w1))
    #     for w1 in W:
    #         # print('w2=' + str(w2))
    ...
    df=pandas.read_excel(os.path.join(dir_name, 'report.xlsx'))
    ...
    df.drop(df[df['Вес скорости достижения цели']])
    fig = plt.figure()
