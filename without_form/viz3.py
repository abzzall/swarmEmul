...
if __name__ == '__main__':
    ...
    file_to_check=open('test3.txt', 'w')
    run = 0
    ...
    for w2 in W:
        ...
        for w1 in W:
            ...
            v, pose, angle, detection, dead, width, height, goalX, goalY, wall, radius, dX, dY, _, t, lX, lY=Env.load_episode_history(ep_file)
            # episode_gui(env, 3, 1)
            # env.episode(w1, w2)
            # env.save_episode(os.path.join(dir_name, str(run) + '_' + str(w1) + '_' + str(w2)))
            alive_agent_count = N * N - numpy.sum(dead[t - 1, :])
            t = float(t) if equals(getDistance(lX, lY, goalX, goalY), 0) else None
