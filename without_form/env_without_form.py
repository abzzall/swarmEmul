...
class Env:
    ...
    def reset(self):
        ...
        for i in range(self.N):
            agent = Agent(
                i,
                self.xL + self.Dx[i], self.yL + self.Dy[i],  radius=self.robot_radius,
    ...
    def play_step(self, w1, w2):
        ...
        for i in range(self.N):
            ...
            v3 = v_goal(self.xL, self.yL, self.xG, self.yG, w2)
            v = w1 * v1 + v3
            # print('i='+str(agent.id)+'v1='+v1.toStr()+', v2='+v2.toStr()+', v3='+v3.toStr()+',v='+v.toStr())
            agent.move(v)
            self.v_history[self.t, agent.id, :, :] = [[v1.x, v1.y],  [v3.x, v3.y], [v.x, v.y]]
            if v.active():
                moving = True
    ...
    def observe(self):
        for i in range(self.N):
            ...
            for j, angle in enumerate(agent.get_lidar_angles()):
                ...
                if min_dist==0:
                    agent.is_dead=True
                elif min_dist < self.sensor_range:
    ...
    def checkGoalAchieved(self):
        return equals(getDistance(self.xL, self.yL, self.xG, self.yG), 0)
...
class Agent(Drawable):
    ...
    def __init__(self, id, x, y,  angle=0, radius=ROBOT_RADIUS, sensor_detection_count=SENSOR_DETECTION_COUNT):
        self.angle = angle
        self.is_dead = False
        self.sensor_detection_count = sensor_detection_count
...
def v_avoid_obs_min_angle(agent: Agent, sensor_range) -> Action:
    if not agent.detected or agent.is_dead:
        return Action(0, 0)
