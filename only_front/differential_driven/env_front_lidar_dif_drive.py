...
from only_front.differential_driven.constants import *
...
class Env:
    ...
    def get_active_agents(self, turn_speed=0):
        ...
        for agent in self.agents:
            ...
            if agent.is_dead:
                continue
            X[agent.id], Y[agent.id] = swifted_axis_coord(agent.x, agent.y, self.xL, self.yL, self.direction+turn_speed)
    ...
    def play_step(self, dv, vmax):
        ...
        obs = self.observe(turn_speed, dv)
        turn = 0
        rand_decision = False
        goal_side = substract_angle(segmentAngleWithXAxis(self.xL, self.yL, self.xG, self.yG), self.direction+turn_speed)
        ...
        #if no obstacle
        if equals(obs[0], obs[1]) and equals(obs[0], obs[1]) and equals(obs[1], self.sensor_range):
            if turning_side == -1 :#and self.turn != -1:
                turn = 1
                # speed_up=False
            elif turning_side == 1:# and self.turn != 1:
                turn = -1
                # speed_up=False

                # self.turn_left=True

            elif goal_side < turn_speed:  # +self.direction:
                turn = 0 if self.turn==-1 else -1
                # speed_up = abs(goal_side) < abs(turn_speed)
                ...
                turn = 0
                # self.turn_left=None
        elif equals(obs[0], obs[2]):
            speed_up=False


            #todo if turning_side==1 and d_obs[2]>0:
            #     turn=0
            # elif turning_side==-1 and d_obs[0]>0:
            #     turn=0
            # elif turning_side==0:
            #     # rand_decision=True
            if turning_side==0:
            ...
            if (turn==1 and turning_side==1 and d_obs[0]>0) or (turn==-1 and turning_side==-1 and d_obs[2]>0):
                speed_up=True
                turn=0
        ...
        if dist<1:
            print('1::',dist, lin_speed, lin_speed//dv)
        if dist<=dv and lin_speed<=dv:
            moving = False
        #
        # if turn == 0:
        ...
        if turn == 0:
            ...
            if turn_speed * turn>= pi / 2:
                turn = -turn
                # speed_up = False
            if (turn==1 and self.v_left>=vmax) or (turn==-1 and self.v_right>=vmax):
                speed_up=False
            elif (turn == 1 and self.v_left <= dv) or (turn == -1 and self.v_right <=dv):
                speed_up = True
            elif abs(range_min)<= ROBOT_RADIUS + abs(lin_speed)+abs(min(d_obs)) +dv:
                speed_up = False
            if speed_up:
                if turn == 1:
                    self.v_left += dv
    ...
    def observe(self, turn_speed, dv, R: float = WHEEL_RADIUS, d: float = L):
        w=dv*R/d
        # if w>0:
        #     wr, wl=w, 0
        # elif w<0:
        ...
        left, center, right = self.get_active_agents(turn_speed)
