...
class Env:
	def __init__(
		...
		sensor_detection_count=SENSOR_DETECTION_COUNT, buffer_size=MAX_T, fuzzy_rules=generate_fuzzy_rules_ang_change()
	):
		# pygame.init()
		self.width = width
		self.height = height
	...
	def play_step(self, v, w):
		...
		#############################################################################################################
		#INCREASE, UNCHANGE, RESET
		fuzzy_result=get_velocity(self.turn_speed, goal_side, obs[0], obs[1], obs[2], w, self.sensor_range)
		if fuzzy_result == 0:
			self.turn_speed+= w if self.turn_speed>0 else -w
		elif fuzzy_result==2:
			self.turn_speed=0
		elif fuzzy_result==3:
			self.turn_speed+=w if bool(random.getrandbits(1)) else -w
		elif fuzzy_result==4:
			self.turn_speed-=w
		elif fuzzy_result==5:
			self.turn_speed+=w
		...
		#     self.direction-=wg
		self.check_dead()
		action = Action(v, self.direction)
		print(action, obs)
		# moving = True
		dist = getDistance(self.xL, self.yL, self.xG, self.yG)
		moving = dist > v and any( not agent.is_dead for agent in self.agents)
		...
		for i in range(self.N):
			...
			self.v_history[self.t, agent.id, 0] = self.turn_speed
			self.v_history[self.t, agent.id, 1] = self.direction
