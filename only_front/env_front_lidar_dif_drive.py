...
class Env:
    ...
    def play_step(self, dv, vmax):
        ...
        range_min = min(obs)
        speed_up=True
        if equals(obs[0], obs[2]) and equals(obs[0], obs[1]) and equals(obs[1], self.sensor_range):
            if goal_side < turn_speed:  # +self.direction:
                turn = -1
        ...
        if turn == 0:
            ...
            if speed_up:
                ...
                if self.v_left >dv and self.v_right >dv:
                    self.v_left -= dv
                    self.v_right -= dv
