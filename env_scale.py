...
class Env:
    ...
    def play_step(self, w1, w2, w3):
        ...
        for i in range(self.N):
            ...
            v2 = v_keep_formation(agent, self.xL, self.yL, self.mean_angle, self.mean_scale, w2)
    ...
    def reset_leader(self):
        self.xL = numpy.array(
            [agent.x + agent.d_dist * self.mean_scale * cos(agent.d_angle + self.mean_angle) for agent in
             self.agents]).mean()
        self.yL = numpy.array(
            [agent.y + agent.d_dist * self.mean_scale * sin(agent.d_angle + self.mean_angle) for agent in
             self.agents]).mean()
        # mean_sin=numpy.array([sin(calculateAngle(self.xL, self.yL , agent.x, agent.y,agent.x-agent.dx, agent.y-agent.dy)) for agent in self.agents]).mean()
        # mean_cos=numpy.array([cos(calculateAngle( self.xL, self.yL , agent.x, agent.y,agent.x-agent.dx, agent.y-agent.dy )) for agent in self.agents]).mean()
        mean_sin = numpy.array(
            [sin(normAngleMinusPiPi(segmentAngleWithXAxis(agent.x, agent.y, self.xL, self.yL)-agent.d_angle)) for agent
             in self.agents]).mean()
        mean_cos = numpy.array(
            [cos(normAngleMinusPiPi(segmentAngleWithXAxis(agent.x, agent.y, self.xL, self.yL)-agent.d_angle)) for agent
             in self.agents]).mean()
        # self.mean_angle= numpy.array([normAngleMinusPiPi( segmentAngleWithXAxis(agent.x, agent.y, old_xl, old_yl) - agent.d_angle) for agent in self.agents]).mean()
        self.mean_angle=atan2(mean_sin,mean_cos)
        #scale_array=numpy.array([getDistance(agent.x, agent.y, old_xl, old_yl)/agent.d_dist if agent.d_dist!=0 else None for agent in self.agents])
       # self.mean_scale=scale_array[scale_array!=None].mean()
...
class Agent(Drawable):
    ...
    def __init__(self, id, x, y, dx, dy, angle=0, radius=ROBOT_RADIUS, sensor_detection_count=SENSOR_DETECTION_COUNT):
        ...
        self.d_dist=vectorLength(self.dx, self.dy)
        self.d_angle=angleWithXAxis(dx, dy)
        self.update_pos()
...
def v_keep_formation(agent: Agent, leader_x, leader_y,mean_angle, mean_scale, w) -> Action:
    if w == 0:
        return Action(0.0, 0.0)
    x_dir = leader_x - (agent.x+agent.d_dist*mean_scale*cos(agent.d_angle+mean_angle))
    y_dir = leader_y - (agent.y+agent.d_dist*mean_scale*sin(agent.d_angle+mean_angle))
