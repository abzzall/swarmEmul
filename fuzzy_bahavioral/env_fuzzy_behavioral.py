...
def v_avoid_obs_min_angle_atan2(agent: Agent, sensor_range) -> Action:
    ...
    for i in range(len(angles)):
        nom += sin(angles[i]) * agent.obs[i]
        denom += cos(angles[i]) * agent.obs[i]
    ...
    k = (sensor_range - min_distance) / sensor_range
    return Action(k * cos(turn_angle), k * sin(turn_angle))
def v_avoid_obs_min_angle_atan2_perp(agent: Agent, sensor_range) -> Action:
    if not agent.detected:
        return Action(0, 0)
    ...
    for i in range(len(angles)):
        ...
        denom += cos(angles[i]) * (sensor_range-agent.obs[i])

    turn_angle = atan2(nom, denom)+pi*0.5
