...
class Env:
    def __init__(
            ...
            sensor_detection_count=4, buffer_size=MAX_T
    ):
        # pygame.init()
        self.width = width
        self.height = height
    ...
    def reset(self):
        ...
        for i in range(self.N):
            agent = Agent(
                ...
                sensor_detection_count=self.sensor_detection_count, angle=segmentAngleWithXAxis(self.xL, self.yL, self.xG, self.yG)
            )
    ...
    def play_step(self, w1, w2, w3):
        ...
        for i in range(self.N):
            ...
            if agent.is_dead:
                continue
            v1 = v_avoid_obs_min_perp(agent, self.sensor_range)
        ...
        self.t += 1
        if not moving or self.t == self.buffer_size or (abs(self.xL - self.xG) < 1 and abs(self.yL - self.yG) < 1):
            self.is_done = True
...
class Agent(Drawable):
    ...
    def __init__(self, id, x, y, dx, dy, angle=0, radius=ROBOT_RADIUS, sensor_detection_count=4):
        self.angle = angle
        self.is_dead = False
        self.sensor_detection_count = sensor_detection_count
    ...
    def get_lidar_angles(self):
        return lidar_angles(self.sensor_detection_count, self.angle, 60)
...
def lidar_angles(sensor_detection_count=SENSOR_DETECTION_COUNT, offset=0, neighbor_angle=None):
    if neighbor_angle == None:
        neighbor_angle = 2 * pi / sensor_detection_count
    from_angle = offset-neighbor_angle * (sensor_detection_count - 1) / 2

    return [
        normAngleMinusPiPi(from_angle + i * neighbor_angle)
        for i in
...
def v_avoid_obs_min_perp(agent:Agent, sensor_range)->Action:
    if not agent.detected:
        return Action(0, 0)
    min_range=agent.obs[0]
    lid_angles=agent.get_lidar_angles()
    min_range_angle=lid_angles[0]
    for i in agent.sensor_detection_count:
        if agent.obs[i] < min_range and agent.obs[i]!=-1:
            min_range=agent.obs[i]
            min_range_angle=lid_angles[i]
    k = (sensor_range - min_range) / sensor_range
    turn_angle= min_range_angle-pi*0.5 if min_range_angle>0 else min_range_angle+pi*0.5
    return Action(k * cos(turn_angle), k * sin(turn_angle))
...
def v_avoid_obs_min_angle_atan2(agent: Agent, sensor_range) -> Action:
    if not agent.detected:
        return Action(0, 0)
    ...
    denom = 0
    for i in range(len(angles)):
        nom += sin(angles[i]) * (sensor_range - agent.obs[i])
        denom += sin(angles[i]) * (sensor_range - agent.obs[i])

    turn_angle = atan2(nom, denom)
